...
namespace YProgramStudio.LabelsDesigner.Barcodes
{
	...
	public class Int104
	{
		private uint[] _byteArray = new uint[13]; // default 0

		public uint[] ByteArray { get => _byteArray; }

		public Int104()
		{
		}

		public void MultUint(uint y)
		{
			ulong carry = 0;
			for (int i = 12; i >= 0; i--)
			{
				ulong temp = ByteArray[i] * y + carry;

				ByteArray[i] = (byte)(temp & 0xFF);
				carry = temp >> 8;
			}
		}

		public void AddUint64(ulong y)
		{
			ulong carry = 0;
			for (int i = 12; i >= 0; i--)
			{
				ulong temp = ByteArray[i] + (y & 0xFF) + carry;
		...
		public uint DivUint(uint y)
		{
			uint carry = 0;
			for (int i = 0; i < 13; i++)
			{
				uint temp = ByteArray[i] + (carry << 8);

				ByteArray[i] = (byte)(temp / y);
				carry = temp % y;
			}
			return carry;
		}
	...
	public class BarcodeOnecode : Barcode1dBase
	{
		...
		readonly ValueTuple<ValueTuple<int, int>, ValueTuple<int, int>>[] barMap =  {
			/*  1 */ ((H, 1<<2 ), (E, 1<<3 )),
			/*  2 */ ((B, 1<<10), (A, 1<<0 )),
			/*  3 */ ((J, 1<<12), (C, 1<<8 )),
			/*  4 */ ((F, 1<<5 ), (G, 1<<11)),
		...
		readonly string[] tdafTable = { "T", "D", "A", "F" };

		readonly uint[] characterTable = {
			/* Table I 5 of 13. */
		...
		protected override string Encode(string cookedData)
		{
			...
			int j;
			for (j = 20; j < cookedData.Length; j++)
			{
				value.MultUint(10);
				value.AddUint64((ulong)(cookedData[j] - '0'));
			}
			...
			value.MultUint(10);
			value.AddUint64((ulong)(cookedData[0] - '0'));
			value.MultUint(5);
			value.AddUint64((ulong)(cookedData[1] - '0'));

			for (int i = 2; i < 20; i++)
			{
				value.MultUint(10);
				value.AddUint64((ulong)(cookedData[i] - '0'));
			}

			/*-----------------------------------------------------------*/
			/* Step 2 -- Generation of 11-Bit CRC on Binary Data         */
			/*-----------------------------------------------------------*/
			uint crc11 = USPS_MSB_Math_CRC11GenerateFrameCheckSequence(value.ByteArray);
			...
			/*-----------------------------------------------------------*/
			codeword[9] *= 2;
			codeword[0] += (uint)(((crc11 & 0x400) != 0) ? 659 : 0);
			...
			{
				((int descenderIdx, int descenderMask), (int ascenderIdx, int ascenderMask)) = bar;

				int d = (character[descenderIdx] & descenderMask) != 0 ? 1 : 0;
				int a = (character[ascenderIdx] & ascenderMask) != 0 ? 1 : 0;
		...
		/// <summary>
		/// USPS_MSB_Math_CRC11GenerateFrameCheckSequence - From Appendix C of USPS publication USPS-B-3200E, 07/08/05.
		/// </summary>
		/// <param name="ByteArray">ByteAttay is the address of a 13 byte array holding 102 bits which are right justified - ie: the leftmost 2 bits of the first byte do not hold data and must be set to zero.</param>
		/// <returns>return unsigned short - 11 bit Frame Check Sequence (right justified)</returns>
		private uint USPS_MSB_Math_CRC11GenerateFrameCheckSequence(uint[] byteArray)
		{
